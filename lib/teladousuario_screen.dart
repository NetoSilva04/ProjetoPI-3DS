...
class _UserPageState extends State<UserPage> {
  ...
  Future<void> _carregarDadosDoUsuario() async {
    // Simula a espera da resposta do banco de dados
    await Future.delayed(const Duration(seconds: 2));

    // Quando conectar ao banco, você irá preencher as variáveis aqui.
    // Por enquanto, elas começam vazias.
    if (mounted) {
      setState(() {
        _redacoes = []; // Começa com uma lista vazia
      });
    }
  }

  Future<void> _excluirRedacao(int index) async {
    // Adicionar lógica de exclusão no banco de dados aqui
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 1,
        actions: [
          IconButton(icon: const Icon(Icons.logout), onPressed: () => Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => const LoginPage()), (route) => false)),
        ],
      ),
